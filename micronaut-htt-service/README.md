...
```bash
aws lambda create-function --function-name micronaut-htt-service \
...
```bash
aws lambda invoke --function-name micronaut-htt-service --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
